...
<%
       ...
       //만약 처음 boardList.jsp를 클릭하거나 수정 삭제 등 다른 게시글에서 이페이지로 넘어오면 pageNum값에 없기에 null처리를 해줌
       if(pageNum == null){
    	   pageNum ="1";
       }
       ...
       int currentPage = Integer.parseInt(pageNum);
        //전체 게시글에 내용을 jsp쪽으로 가저와야 함
        //BoardDAO 보드의 게시글이 몇개인지 확인  할 수 없기때문에 가변길이인 ArrayList 
        BoardDAO bdao = new BoardDAO();
        //전체 게시글의 겟수를 읽어드린 메소드 호출
        count  = bdao.getAllCount();
        
        //현재 페이지에 보여질 시작 번호를 설정 = 데이터베이스에서 불러올 시작번호를 의미
        // 예) 67~77
...
<table width="600" border="1" bgcolor="skyblue">
   ...
   <%
         for(int i=0;i<list.size();i++){
        	 //패키징 된 가방을 하나씩 추출해야 한다. 
        	  BoardBean bean = list.get(i); 
        	 // array에 저장되어 있는 빈 클래스를 하나씩 추출한다. 
      %> 
        	  <tr height="40">
                 <%-- <td width="50"  align="center"><%=i+1 %></td> --%>
                 <td width="50"  align="center"><%=number-- %></td>
                 <td width="250"  align="left"><a href="BoardInfo.jsp?num=<%= bean.getNum()%>" style="text-decoration:none">
                 <%
...
<%
      if(count  > 0){
    	  ...
    	  int pageBlock = 10 ; //카운터링 처리숫자를 의미함.
...
%>
<br>
     현재보여지고 있는 페이지 넘버값을 읽어들임pageNum :<%=pageNum  %><br>
     넘버링 숫자값number :<%=number  %><br>
     현재페이지currentPage :<%=currentPage %><br>
      전체 게시글 개수count :<%=count %><br>
       한 화면에 출력할 페이지pageSize :<%=pageSize %>
</center>
</body>
</html>
