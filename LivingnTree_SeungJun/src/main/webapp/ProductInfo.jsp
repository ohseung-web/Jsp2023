...
.tabProduct ul li a, .tabProduct ul li button{
	position: relative;
    display: inline-block;
    font-weight: 400;
    font-size: 20px;
...
.tabProduct ul li.selected a, .tabProduct ul li.selected button{
	font-weight: 600;
    color: #d0ac88;
    background-color: #fff;
    border-bottom: 2px solid #d0ac88;
}
.tabProduct ul li a span, .tabProduct ul li button span {
	position: absolute;
    top: -10px;
    right: -10px;
    display: block;
...
/* 오티수정 REVIEW  CSS 수정 시작 */
.prdReview .reviewContainer{
    position: relative;
    display: flex;
    justify-content: center;
    width: 100%;
}
.prdReview .reviewContainer .contents{
    max-width: 1480px;
    width: 92%;
    margin: 50px 0;
}
.prdReview .reviewContainer .contents .titleArea{
    margin: 55px 0 40px;
    text-align: center;
}
.prdReview .titleArea h2{
    display: inline-block;
    word-break: keep-all;
    word-wrap: break-word;
    width: 100%;
...
.prdReview .titleArea p{
    max-width: 586px;
    margin: 25px auto 0;
    font-size: 14px;
    color: #6D6D6D;
}
.prdReview .reviewContainer .contents .typeList{
    border-top: 0;
}
.prdReview table{
    width: 100%;
...
.prdReview .typeList table th, .typeList table td{
    height: 29px;
...
.prdReview .typeList table td{
    border-bottom: 1px solid #e5e5e5;
    color: #555;
    text-align: center;
}
.prdReview .typeList table td.code{
    color: #6d6d6d;
}
.prdReview .typeList table td.subject{
    padding-left: 20px;
    padding-right: 20px;
    text-align: left;
    word-break: break-all;
...
.prdReview .typeList table td.subject a{
    margin-right: 5px;
    vertical-align: middle;
}
.prdReview .typeList .boardEmpty{
    padding: 56px 0;
    border-bottom: 1px solid #e5e5e5;
    text-align: center;
    color: #6d6d6d;
}
.prdReview .typeList .boardButton{
    margin-top: 20px;
    text-align: center;
}
.prdReview .btnSubmitFix{
    display: inline-block;
    padding: 12px 20px;
    height: 40px;
    font-size: 13px;
...
.prdReview .typeList .boardButton .btnSubmitFix{
    float: right;
    width: 135px;
    word-break: keep-all;
    word-wrap: break-word;
}
.prdReview .typeList #boardSearchForm{
    margin-top: -10px;
}
.prdReview .typeList #boardSearchForm .boardSearch{
    max-width: 1480px;
    margin: 20px -5px 0;
    margin-left: auto;
    margin-right: auto;
}
.prdReview .boardSearch .form{
    display: flex;
}
.prdReview .boardSearch select, .boardSearch input[type="text"], .boardSearch a{
    margin: 0 5px;
}
...
.prdReview .typeList .pageing{
    margin: 50px auto;
    text-align: center;
}
.prdReview .typeList .pageing > a:first-child{
    background: url(img/icon/btn_page_prev.png) no-repeat center center;
}
.prdReview .typeList .pageing > a{
    display: inline-block;
    width: 40px;
    height: 40px;
    line-height: 40px;
...
.prdReview .typeList .pageing ol{
    display: inline-block;
    font-size: 0;
    line-height: 0;
    vertical-align: middle;
}
.prdReview .typeList .pageing li{
    display: inline-block;
    margin: 0 0 0 -1px;
    vertical-align: top;
}
.prdReview .typeList .pageing li a{
    display: block;
    width: 40px;
    padding: 12px 0;
    font-size: 14px;
                    ...
                    <div class="tabProduct">
                        <ul>
                            ...
                            <li class>
                                ...
                                <%-- <a href="#">Review<span>${count}</span></a> --%>
                                <!-- 리뷰 게시판 작성완료 후 수정완료 -->
                            </li>
                            <li class>
						                ...
						                <c:if test="${aList.size() ne 0}">
						                    <div class="pageing"> 
						                        <c:if test="${startPage>pageBlock }">
						                            <a href="ProductInfo.do?pageNum=${startPage-pageBlock}">이전</a>
						                        </c:if>
						                        
						                        <ol>
						                            <c:forEach var="i" begin="${startPage}" end="${endPage}">
						                                <li>
						                                    <a href="ProductInfo.do?pageNum=${i}">${i}</a>
						                                </li>
						                            </c:forEach>
						                        </ol>
						    
						                        <c:if test="${endPage<pageCount}">
						                            <a href="ProductInfo.do?pageNum=${startPage+pageBlock}">다음</a>
						                        </c:if>
						                    </div>
						                </c:if>
						            </div>
