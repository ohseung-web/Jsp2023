...
public class RentDAO {
 ...
 // memberjoin 테이블을 이용하여 회원 비밀번호를 리턴하는 메소드
    ...
    public int getAllCount(int category) {
    	...
    	int count = 0;
    	try {
    	     String sql ="select count(*) from rentcar where category=?";
    	     pstmt = con.prepareStatement(sql);
    ...
    public ArrayList<RentDTO> getAllRentcar(int category, int start, int pageSize){
    	
    	getConnect();
    	ArrayList<RentDTO> a = new ArrayList<RentDTO>();
    	
    	try {
   	         String sql ="select * from rentcar where category=? order by no desc limit ?,?";
   	         pstmt = con.prepareStatement(sql);
   	         pstmt.setInt(1, category);
