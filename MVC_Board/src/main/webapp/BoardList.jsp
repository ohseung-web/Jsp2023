      ...
      <c:forEach var="mdto" items="${alist}">
       ...
       <tr height="40" >
        ...
        <td width="320" align="left">
          <c:if test="${mdto.re_step > 1 }">
          <c:forEach var="j" begin="1" end="${(mdto.re_step-1)*3}">
             &nbsp;
          </c:forEach>
          </c:if>
          <a href="BoardInfoControl.do?num=${mdto.num}">${mdto.subject}</a>
        </td>
        <td width="100" align="center">${mdto.writer}</td>
        <td width="150" align="center">${mdto.reg_date}</td>
        <td width="80" align="center">${mdto.readcount}</td>
     </tr> 
      <!-- nujmber-1을 이용하여 1씩 감소한다.  number=10이면 다음 number=9가된다 -->
