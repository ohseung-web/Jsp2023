 ...
 // 모든 장바구니의 정보를 검색하는 메소드
    public ArrayList<JangDTO> getAllJang(){
    	...
    	try {
    		 ...
    		 String sql = "select * from rentjang2 order by no desc";
    ...
    public void insertJang(JangDTO jdto){
    	...
    	try {
   		 ...
   		 String sql = "insert into rentjang2 values(?,?,?,?,?)";
   		 pstmt = con.prepareStatement(sql);
    ...
    public int getNoCount(int no) {
    	...
    	try {
      		 String sql = "select count(*) from rentjang2 where no=?";
      		 pstmt = con.prepareStatement(sql);
      	     pstmt.setInt(1, no);
      	     rs = pstmt.executeQuery();
      	     if(rs.next()) {
    ...
    public JangDTO updateNoCount(int cnt, int no) {
    	...
    	JangDTO jdto = new JangDTO();
    	try {
     		 String sql = "update rentjang2 set cnt = cnt + ? where no=?";
     		 pstmt = con.prepareStatement(sql);
     		 pstmt.setInt(1, cnt);
     	     pstmt.setInt(2, no);
     	     pstmt.executeUpdate();
    ...
    public void deletejang(int no) {
    	...
    	try {
    		 String sql = "delete from rentjang2 where no=?";
    		 pstmt = con.prepareStatement(sql);
    		 pstmt.setInt(1, no);
    	     pstmt.executeUpdate();
 		} catch (Exception e) {
    ...
    public JangDTO updatecount(int cnt, int no) {
    	...
    	JangDTO jdto = new JangDTO();
    	try {
     		 String sql = "update rentjang2 set cnt=?  where no=?";
     		 pstmt = con.prepareStatement(sql);
     		 pstmt.setInt(1, cnt);
     	     pstmt.setInt(2, no);
     	     pstmt.executeUpdate();
    ...
    public JangDTO buyselect(int no){
    	...
    	try {
    		 String sql = "select * from rentjang2 where no=?";
    		 pstmt = con.prepareStatement(sql);
    		 pstmt.setInt(1, no);
    	     rs = pstmt.executeQuery();
   ...
   // 구매한 상품 한개의 배송자 정보를 저장 (insert)하는 메소드를 작성
    ...
    public int rentjangCount() {
    	...
    	try {
			...
			String sql = "select count(*) from rentjang2";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if (rs.next()) {
