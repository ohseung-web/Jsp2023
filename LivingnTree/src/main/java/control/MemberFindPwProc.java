...
@WebServlet("/MemberFindPwProc.do")
public class MemberFindPwProc extends HttpServlet {
	...
	protected void reqPro(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(check_method.equals("email")) {
			...
			mdto = ldao.getMemberByEmail(id,name,email);
			if(mdto.getM_id() == null) {
				request.setAttribute("msg", "입력하신 정보로 가입된 회원은 존재하지 않습니다.");
			...
			mdto = ldao.getMemberByPhone(id,name,phone);
			if(mdto.getM_id() == null) {
				request.setAttribute("msg", "입력하신 정보로 가입된 회원은 존재하지 않습니다.");
				RequestDispatcher rd = request.getRequestDispatcher("MemberFindPwError.jsp");
				rd.forward(request, response);
