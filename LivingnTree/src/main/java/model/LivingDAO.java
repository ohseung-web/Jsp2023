...
public class LivingDAO {
	...
	public ArrayList<ProductDTO> getAllProduct(int category, int startRow, int pageSize){
		...
		try {
			...
			pstmt.setInt(2, startRow-1); //index는 0부터시작 하므로 반드시 -1한다.
	...
	// 카테고리별 높은 가격순으로 정렬하는 메서드
	public ArrayList<ProductDTO> highPrice(int category) {
		getConnect();
		ArrayList<ProductDTO> a = new ArrayList<>();
		try {
			String sql = "select * from product where p_category=? order by p_price desc";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, category);
			rs = pstmt.executeQuery();
			while(rs.next()) {
	...
	// 카테고리별 낮은 가격순으로 정랼하는 메서드
	public ArrayList<ProductDTO> lowPrice(int category) {
		getConnect();
		ArrayList<ProductDTO> a = new ArrayList<>();
		try {
			String sql = "select * from product where p_category=? order by p_price asc";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, category);
			rs = pstmt.executeQuery();
			while(rs.next()) {
